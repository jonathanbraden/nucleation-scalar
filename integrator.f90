...
module integrator
  ...
  implicit none
#ifdef BENCHMARK
  type(myClock) :: matTime, derivTime, vecTime
#endif
  integer :: numVar
...
#ifdef USEBLAS
  real*8, allocatable, dimension(:,:) :: gtmp
#endif
#else
  real*8, dimension(nvar,order) :: g
#ifdef USEBLAS
  real*8, dimension(nvar,order) :: gtmp
#endif
#endif

  integer, parameter :: niter = 8
  real*8, parameter :: a(order,order) = reshape( (/ &
...
#ifdef BENCHMARK
  subroutine resetIntegratorClocks()
    call reset(matTime)
    call reset(derivTime)
    call reset(vecTime)
  end subroutine resetIntegratorClocks

  subroutine integratorBenchmarks()
    print*,"Time in matmul is ",total_time(matTime)
    print*,"Time in derivs is ",total_time(derivTime)
    print*,"Time in dgemv is ",total_time(vecTime)
  end subroutine integratorBenchmarks
#endif
